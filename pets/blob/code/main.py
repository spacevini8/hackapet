...
blob_sprite = displayio.TileGrid(
    blob_sheet,
...
def update_blob():
    global blob_sprite
    try:
        new_bitmap = displayio.OnDiskBitmap(blob_states[current_mode][rotation_index])
        new_sprite = displayio.TileGrid(
            new_bitmap,
            pixel_shader=new_bitmap.pixel_shader,
            width=1,
            height=1,
            tile_width=tile_width,
            tile_height=tile_height,
        ...
        blob_sprite = new_sprite
        splash.append(blob_sprite)
    except KeyError:
        print(f"Error: Invalid mode '{current_mode}' or rotation '{rotation_index}'")
