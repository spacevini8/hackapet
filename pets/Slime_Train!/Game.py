...
while True:
    ...
    if check_collision(Slime_Sprite, Merchant_All[2]) and Up and Pet.y==display.height - 16 - Pet.tile_height and area[0]==1:
        if Gold>=50:
            Gold-=50
            createobject(Snack_All, random.randint(10, 100), 0, 1, random.randint(0, Snack_All[1].width//9), 9)
    if check_collision(Slime_Sprite, Witch_Merchant_All[2]) and Up and Pet.y==display.height - 16 - Pet.tile_height and area[0]==3:
        if Gold>=100:
            Gold-=100
            createobject(Potions_All, random.randint(10, 100), 0, 3, random.randint(0, (Potions_All[1].width//9)-1), 9)
    if area[0]==1:
